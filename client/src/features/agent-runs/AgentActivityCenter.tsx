    ...
    <article className="grid gap-3 px-4 py-3 md:grid-cols-[minmax(0,1fr)_auto] md:items-center">
      <div className="min-w-0">
        <div className="flex min-w-0 flex-wrap items-center gap-2">
          <p className="min-w-0 truncate text-[13px] font-extrabold text-ink">
        ...
        <p className="mt-1 line-clamp-1 text-xs leading-5 text-gray-500">{runStep(agentRun, proposal)}</p>
        {proposal ? (
      ...
      <div className="flex flex-wrap items-center gap-2 md:justify-end">
        ...
        <span className="self-center whitespace-nowrap text-[11px] font-semibold text-gray-400">
...
function ActivityGroupSection({
  ...
  return (
    <section className="overflow-hidden rounded-xl border border-gray-200 bg-white">
      <button
        className="flex h-14 w-full items-center gap-3 border-b border-gray-200 bg-slate-50 px-4 text-left"
        onClick={onToggle}
        type="button"
      >
        ...
        <span className="min-w-0 flex-1">
          ...
          <span className="block text-[11px] font-semibold text-gray-500">
            {items.length ? `${items.length} run${items.length > 1 ? 's' : ''}` : copy.empty}
          </span>
        </span>
        <span className="rounded-full border border-gray-200 bg-white px-2 py-0.5 text-xs font-bold text-gray-500">
          {items.length}
        </span>
        <ChevronDown
          className={`text-gray-400 transition ${isCollapsed ? '-rotate-90' : 'rotate-0'}`}
          size={16}
        />
      </button>

      {isCollapsed ? null : items.length ? (
        ...
        <p className="px-4 py-5 text-sm leading-6 text-gray-500">{copy.empty}</p>
      )}
    </section>
  )
}
...
export function AgentActivityPage({
  ...
  const [collapsedGroups, setCollapsedGroups] = useState<Record<ActivityGroup, boolean>>({
    running: false,
    needsReview: groups.needsReview.length === 0,
    failed: false,
    completed: false,
  })

  const toggleGroup = (group: ActivityGroup) => {
        ...
        <div className="flex flex-wrap items-start justify-between gap-6">
          ...
          <div className="flex flex-wrap gap-2">
            {[
              ...
              <div
                className="flex h-10 items-center gap-2 rounded-lg border border-gray-200 bg-slate-50 px-3"
                key={label}
              >
