...
- Agent run SSE validation: added `/agent-runs/stream` with named events (`agent-stream.connected`, `agent-run.queued`, `agent-run.event`, `agent-run.completed`, etc.) and replaced the client 3-second workspace polling loop with EventSource-triggered debounced refreshes. `npm run typecheck --workspace=server`, `npm run typecheck --workspace=client`, `npm run test --workspace=server`, `npm run build`, and `git diff --check` pass. Browser smoke confirmed no API calls after waiting past the old poll interval, then one coalesced workspace refresh after a pushed `reindex_links` run event.
- Agent Activity list validation: redesigned the Agent Activity page from a 2x2 section grid into a single-column collapsible listing. `npm run typecheck --workspace=client` and `npm run build --workspace=client` pass. Browser smoke confirmed the page uses the single-column list, no longer renders the `xl:grid-cols-2` layout, section headers collapse rows, and no console warnings/errors appear.
