...
import type { GeneralKnowledgeExtraction } from "../../domain/compiler.js";
...
export type CompileRawNoteAgentRuntimeInput = {
  agentRunId: string;
  rawSourceId: string;
  ...
  wikiIndexerService: WikiIndexer;
  compileAgentRunnerFactory: CompileAgentRunnerFactory;
};

export type CompileRawNoteAgentRuntimeResult = {
  proposalOutput: Awaited<ReturnType<AgentToolService["draftProposal"]>>;
  extraction: GeneralKnowledgeExtraction;
};

export async function runCompileRawNoteAgentRuntime(
  input: CompileRawNoteAgentRuntimeInput,
): Promise<CompileRawNoteAgentRuntimeResult> {
  ...
  const indexConcepts = async () => {
    ...
    for (const concept of loopState.extraction.structuredData.concepts) {
      const savedConcept = await input.knowledgeRepository.upsertConcept({
        userId: input.source.userId,
        name: concept.name,
        conceptType: concept.type,
      });
      loopState.conceptIds.push(savedConcept.id);
      await input.knowledgeRepository.indexConcept({
        userId: input.source.userId,
        conceptId: savedConcept.id,
        targetType: "raw_source",
        targetId: input.rawSourceId,
  ...
  const classifyOutcome = async (view: LoopView) => {
    ...
    if (!loopState.outcomeClassified) {
      if (loopState.extraction.outcome !== "keep_searchable" && input.wikiIndexerService.classifyOutcome) {
        const classification = await input.wikiIndexerService.classifyOutcome({
          source: input.source,
      ...
      await input.agentRunRepository.addEvent({
  ...
  const buildDraftInput = (
    ...
    refreshLoopObservations(loopState, view);
    const draft = input.wikiIndexerService.draftProposal(input.source, loopState.extraction, input.relatedNotes);
    const proposalChunks = loopState.sourceToolOutput?.chunks ?? contractChunks(input.source);
    ...
    const fallbackReason = options.reason
      ? `${loopState.extraction.outcomeReason} ${options.reason}`
      : loopState.extraction.outcomeReason;
    const items = draft.items
      ...
      .map((item) => {
        ...
        return {
          ...
          title: typeof payload.title === "string" ? payload.title : input.source.title ?? "Untitled knowledge",
          body_markdown: typeof payload.bodyMarkdown === "string" ? payload.bodyMarkdown : input.source.bodyMarkdown,
  ...
  const runDraftProposal = async (draftInput: DraftProposalInput, view: LoopView) => {
    refreshLoopObservations(loopState, view);
    await indexConcepts();
    loopState.extraction = {
      ...loopState.extraction,
      outcome: draftInput.indexing_outcome,
      outcomeReason: draftInput.outcome_reason,
    };
    await input.rawSourceRepository.updateExtraction(input.rawSourceId, loopState.extraction);
    const proposalOutput = await input.agentToolService.draftProposal(
      {
        agentRunId: input.agentRunId,
        rawNoteId: input.source.rawNoteId,
        sourceId: input.source.rawSourceId ?? input.source.id,
        userId: input.source.userId,
        sourceText: input.source.bodyMarkdown,
        chunks: loopState.sourceToolOutput?.chunks ?? contractChunks(input.source),
    ...
    await input.agentRunRepository.addEvent({
      ...
      payload: {
        ...
        suggestedCount: draftInput.suggested_links.length,
  ...
  const tools = createCompileLoopTools({
    agentRunId: input.agentRunId,
    agentRunRepository: input.agentRunRepository,
    source: input.source,
    loopState,
    agentToolService: input.agentToolService,
    runDraftProposal,
    buildDraftInput,
    classifyOutcome,
  });
  const runner = input.compileAgentRunnerFactory({
    source: input.source,
    extraction: input.extraction,
    extractedConceptNames: input.extractedConceptNames,
    classifyOutcome,
    buildDraftInput,
  });
  const loopOutcome = await runAgentLoop({
    ...
    maxRounds: 8,
    maxCallsPerTool: 3,
    onEvent: (event) => recordLoopEvent(input.agentRunRepository, input.agentRunId, event),
  });
  ...
  return {
    proposalOutput: loopState.proposalOutput!,
    extraction: loopState.extraction,
  };
}

function createCompileLoopTools(input: {
  agentRunId: string;
  agentRunRepository: AgentRunRepository;
  source: WikiIndexingSource;
  ...
  return [
    ...
    },
    {
      ...
      run: async (toolInput, view) => {
        ...
        await recordOutcomeClassified(input.agentRunRepository, input.agentRunId, loopState);
...
async function recordOutcomeClassified(
  agentRunRepository: AgentRunRepository,
  agentRunId: string,
  loopState: CompileLoopState,
) {
  ...
  loopState.outcomeClassified = true;
  await agentRunRepository.addEvent({
    agentRunId,
    ...agentRunEvents.indexing.outcomeClassified,
    payload: {
      outcome: loopState.extraction.outcome,
 ...
 */
export function createScriptedCompileAgentRunner(context: CompileAgentRunnerContext): AgentRunner {
