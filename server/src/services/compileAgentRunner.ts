...
function buildInstructions() {
  return [
    ...
    "- If you intend to set target_block_id, you MUST call get_block for that exact block before draft_proposal. A search preview is not enough for updating.",
    ...
    "- target_block_id means the proposal body is a complete replacement version for that existing knowledge, not a delta.",
    ...
    "- For a targeted update, preserve the existing block's useful definition, explanation, caveats, and examples unless the new source explicitly revises them; add source-supported material into the right place.",
    "- For a targeted update, cite the existing target block id in structured_facets.claims.evidenceChunkIds for preserved approved claims, and cite new source chunk ids for newly added claims or examples.",
    ...
    "- Never submit only the new source, delta, or example as body_markdown when target_block_id is set; body_markdown must stand alone as the full merged note.",
